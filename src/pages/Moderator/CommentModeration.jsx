    ...
    try {
      ...
      const enriched = await Promise.all(
        normalized.map(async (c) => {
          const profile = c.user_id ? await getUserProfile(c.user_id) : null;
          return { ...c, authorLabel: profile?.display_name || profile?.handle || "a member" };
        })
      );
      setQueue(enriched);
    } catch (err) {
      ...
      {isLoading ? (
        ...
        queue.map((comment) => (
          <div key={comment.id} className="moderation-item">
            ...
            <p className="moderation-meta">Submitted by {comment.authorLabel}</p>
            <div className="moderation-actions">
              <button onClick={() => approve(comment.id)} disabled={processingId === comment.id}>Approve</button>
              <button onClick={() => reject(comment.id)} disabled={processingId === comment.id}>Reject</button>
            </div>
          </div>
        ))
      )}
