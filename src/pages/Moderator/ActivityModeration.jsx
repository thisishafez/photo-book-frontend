...
export default function ActivityModeration() {
  ...
  return (
    <div className="activity-moderation-page">
      <h1>Activity Moderation Queue</h1>
      ...
      {isLoading ? (
        ...
        queue.map((activity) => (
          <div key={activity.ID} className="moderation-item">
            <h3>{activity.Title}</h3>
            <p>{activity.Description || "No description."}</p>
            <p className="moderation-meta">Submitted by {activity.creatorLabel}</p>
            <div className="moderation-actions">
              <button onClick={() => approve(activity.ID)} disabled={processingId === activity.ID}>Approve</button>
              <button onClick={() => reject(activity.ID)} disabled={processingId === activity.ID}>Reject</button>
            </div>
          </div>
        ))
      )}
